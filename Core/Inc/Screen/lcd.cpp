...
{	
	...
	int srSqrd = SR * SR;
	int lrSqrd = LR * LR;
	int dblSrSqrd = 2 * srSqrd;
	int dblLrSqrd = 2 * lrSqrd;
	int decisionParameter;
	int deltaX = 0;
	int deltaY = dblLrSqrd * y;

	// Region 1: From the top of the ellipse to the point where the slope is -1
	decisionParameter = lrSqrd - (srSqrd * SR) + (0.25 * srSqrd);
	while (deltaX < deltaY) {
		// Draw points in all four quadrants
		LCD_DrawDot(centerX + x, centerY + y, usColor);
		LCD_DrawDot(centerX - x, centerY + y, usColor);
		LCD_DrawDot(centerX + x, centerY - y, usColor);
		LCD_DrawDot(centerX - x, centerY - y, usColor);
		...
		if (decisionParameter < 0) {
			decisionParameter += lrSqrd + deltaX;
		} else {
			y--;
			deltaY -= dblSrSqrd;
			decisionParameter += lrSqrd + deltaX - deltaY;
		}
	}

	    // Region 2: From the point where the slope is -1 to the bottom of the ellipse
	    decisionParameter = lrSqrd * (x + 0.5) * (x + 0.5) + srSqrd * (y - 1) * (y - 1) - srSqrd * lrSqrd;
	while (y > 0) {
		// Draw points in all four quadrants
		LCD_DrawDot(centerX + x, centerY + y, usColor);
		LCD_DrawDot(centerX - x, centerY + y, usColor);
		...
		deltaY -= dblSrSqrd;
		if (decisionParameter > 0) {
			decisionParameter += srSqrd - deltaY;
		} else {
			x++;
			deltaX += dblLrSqrd;
			decisionParameter += srSqrd - deltaY + deltaX;
		}
	}
}
